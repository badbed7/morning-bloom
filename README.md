...
- 해바라기 5일·라벤더 7일·은방울꽃 14일의 장기 재배, 접속 종료 중에도 개화까지 성장
- 레퍼런스 기반 주말용 팬지 2일·코스모스 3일·프리지아 4일, 첫 물주기 후 접속 없이 개화
- 상단 게이지와 오른쪽 빈 화분을 향해 누르고 이동하는 민들레 씨앗 여행, 무강화 누적 2시간
- 화분 도착 즉시 별꽃 판매 속도의 2시간 가치인 8G 지급, 진행도 자동 저장과 중복 보상 방지
- 데이지 꽃머리 게임 아이콘, PNG·Windows ICO와 실행 창·배포 빌드 적용
...
- 재배 중인 화분 그림의 우클릭 플로팅, 성장 모습·수확 후 위치 유지
...
- 일반 재배 72시간 오프라인 상한, 장기 꽃은 전체 성장 시간까지 반영, 휴가 모드는 성장 정지
- v1~v11 저장의 v12 이전, 기존 재배는 수확까지 옛 규칙 유지, 30초·행동·정상 종료 저장과 직전 정상 백업
...
- `.json.v8-migration.bak`: v8에서 v9로 처음 이전할 때의 원문
- `.json.v9-migration.bak`: v9에서 v10으로 처음 이전할 때의 원문
- `.json.v10-migration.bak`: v10에서 v11로 처음 이전할 때의 원문
- `.json.v11-migration.bak`: v11에서 v12로 처음 이전할 때의 원문
