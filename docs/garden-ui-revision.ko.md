...
후속 수정: 2026-09-16 개별 재배 화분의 선택 드롭다운도 순환 슬라이드로 변경했다. 아래 3.1절과 6절이 이번 추가 범위다.
...
설정은 이 순환에 포함하지 않는다. 오른쪽 아래의 설정 버튼을 모든 화면에 유지한다. 설정에서 `돌아가기`를 누르면 설정을 열기 직전 화면으로 돌아간다. 씨앗 품종 선택용 드롭다운은 유지하며, 개별 화분 선택은 아래와 같이 변경한다.
...
- 양쪽 화살표를 누르면 해당 방향으로 약 180ms 슬라이드하며 이전/다음 화분의 그림·성장 정보·돌봄 도구를 보여준다. 마지막 다음은 첫 화분, 첫 화분 이전은 마지막 화분이다.
...
- 현재 구매 상한은 두 개로 유지한다. 후속 네/여섯 화분 확장에서도 같은 좌우 순환 방식을 사용한다.
