...
class TycoonUI(unittest.TestCase):
    ...
    def test_double_click_keyboard_and_timeout_do_not_duplicate_rounds(self):
        ...
        with patch('morning_bloom.fertilizer_game.random.uniform', return_value=.35), \
                patch('morning_bloom.fertilizer_game.time.monotonic', return_value=100) as clock:
            QTest.keyClick(game.start_button, Qt.Key_Space)
    ...
    def test_game_inherits_and_tracks_opacity_and_keeps_window_controls(self):
        self.window.opacity.setValue(45)
        self.window.open_fertilizer_game()
        game = self.window._fertilizer_game
        self.app.processEvents()
        self.assertAlmostEqual(game.windowOpacity(), self.window.windowOpacity(), places=2)
        self.assertAlmostEqual(game.windowOpacity(), .45, delta=.01)
        self.assertTrue(game.windowFlags() & Qt.FramelessWindowHint)
        self.assertEqual(bool(game.windowFlags() & Qt.WindowStaysOnTopHint),
                         bool(self.window.windowFlags() & Qt.WindowStaysOnTopHint))
        self.window.opacity.setValue(80)
        self.assertAlmostEqual(game.windowOpacity(), self.window.windowOpacity(), places=2)
        for width in (300, 320, 384):
            game.setFixedWidth(width)
            self.app.processEvents()
            for widget in (game.close_button, game.start_button, game.bar, game.hint):
                self.assertTrue(widget.isVisible())
                self.assertTrue(game.rect().contains(widget.mapTo(game, widget.rect().bottomRight())))
        self.play_success(game)
        self.app.processEvents()
        self.assertTrue(game.return_button.isVisible())
        self.assertTrue(game.result.isVisible())
        self.assertFalse(game.stop_button.isVisible())
        self.assertEqual([marker.text() for marker in game.round_markers], ['1 성공', '2 성공', '3 성공'])
        game.return_button.click()
        self.assertIsNone(self.window._fertilizer_game)
        self.window.open_fertilizer_game()
        QTest.keyClick(self.window._fertilizer_game, Qt.Key_Escape)
        self.assertIsNone(self.window._fertilizer_game)
    ...
    def test_four_pot_layout_and_summary_at_three_sizes(self):
        ...
        for side in (320, 384, 520):
            ...
            for widget in (self.window.water_button, self.window.mist_button, self.window.fertilizer_button,
                           self.window.make_fertilizer_button, self.window.fertilizer_stock):
                self.assertTrue(widget.isVisible())
                self.assertTrue(self.window.rect().contains(widget.mapTo(self.window, widget.rect().bottomRight())))
            self.assertFalse(self.window.harvest_button.isVisible())
            self.assertFalse(self.window.plant_button.isVisible())
        ...
        for button in (self.window.water_button, self.window.mist_button, self.window.fertilizer_button):
        ...
        self.window.refresh()
        self.app.processEvents()
        self.assertTrue(self.window.harvest_button.isVisible())
        self.assertTrue(self.window.harvest_button.isEnabled())
        self.assertTrue(self.window.make_fertilizer_button.isVisible())
        for button in (self.window.water_button, self.window.mist_button, self.window.fertilizer_button):
            self.assertFalse(button.isVisible())
